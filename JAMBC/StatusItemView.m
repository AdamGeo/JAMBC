...
-(void)rightMouseUp:(NSEvent *)event {
    ...
    [theMenu insertItemWithTitle:@"Operators" action:@selector(showHelp:) keyEquivalent:@"" atIndex:0];
    [theMenu insertItemWithTitle:@"Quit" action:@selector(quitCalcyBar:) keyEquivalent:@"" atIndex:1];
    [NSMenu popUpContextMenu:theMenu withEvent:event forView:self];
}

-(void)quitCalcyBar:(id)sender {
    [NSApp terminate:self];
}

-(void)showHelp:(id)sender {
    [NSApp sendAction:@selector(showOps:) to:self.target from:self];
}
